...
class TestInjectionProcess(TestCase):
    ...
    def test_meal_without_iob(self):
        ...
        rows = self.cloop_db.select("select units_intended, units_delivered, temp_rate, status from injections")
        if len(rows) > 1:
        ...
        rows = self.cloop_db.select("select iob, datetime_iob from iob")
        self.assertEqual(len(rows), 0, "Fail: Test 1: wrong number of rows for iob (" + str(len(rows)) + ")")

        rows = self.cloop_db.select("select * from alerts")
        if len(rows) != 2:
